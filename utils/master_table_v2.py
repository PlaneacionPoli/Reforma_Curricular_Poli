"""
Tabla maestra V2, y exportación Excel — agrupada por formato/proceso.
Basado en utils/vact_master_table.py, adaptado a los 11 formatos de la hoja
"Etapas (2)".
"""
...
def _status_legend_html() -> str:
    ...
    return (
        ...
        "· Pulse + en cada formato para ver los campos selector</span></div>"
    )
...
def _blend_with_white(hex_color: str, tint: float) -> str:
    ...
    w = 1.0 - tint
    return f"#{int(255 * w + r * tint):02x}{int(255 * w + g * tint):02x}{int(255 * w + b * tint):02x}"


def _formato_hdr_tints(clr: str) -> dict[str, str]:
    return {
        "etapa_bg": clr,
        "etapa_fg": "#FFFFFF",
...
.vact-master-table thead tr.hdr-etapa th {{
  position: sticky; top: 0; z-index: 4; height: {_HDR_ROW1_H}px; padding: 5px 8px;
  border-bottom: none; background-clip: padding-box; vertical-align: middle;
}}
.vact-master-table thead tr.hdr-act th {{
  position: sticky; top: {_HDR_ROW1_H}px; z-index: 3; padding: 5px 5px;
  border-top: none; background-clip: padding-box; vertical-align: bottom; opacity: 1;
}}
.vact-master-table td[class^="col-act-"] {{ opacity: 1; }}
.vact-master-table thead th.th-pin-left {{ left: 0; z-index: 6 !important; box-shadow: 2px 0 4px rgba(0,0,0,.06); }}
.vact-master-table thead th.th-pin-gen {{ z-index: 5 !important; }}
</style>
"""
...
def master_activities_table_html(df: pd.DataFrame) -> str:
    ...
    for formato in FORMATOS_ORDEN:
        meta = [m for m in meta_all if m["phase"] == formato]
        clr_formato = FORMATO_CLR.get(formato, BRAND_PRIMARY)
        formato_blocks.append({
            "formato": formato,
            "slug": FORMATO_SLUG[formato],
            "meta": meta,
            "pct_col": FORMATO_PCT_COL[formato],
            "tints": _formato_hdr_tints(clr_formato),
        })
        ncols += 1 + len(meta)
    ncols += 1
    ...
    for blk in formato_blocks:
        slug, formato, meta = blk["slug"], blk["formato"], blk["meta"]
        t = blk["tints"]
        n_span_expanded = 1 + len(meta)
        short = FORMATO_SHORT.get(formato, formato)
        pct_lbl = f"% {_short_label(short, 18)}"
        h1.append(
    ...
    for _, row in df_sorted.iterrows():
        ...
        cells.append(
            f'<td class="col-pct-general" {TD} style="border-left:2px solid {BORDER_ROW}">{p_bar_html(gen_pct)}</td>'
        )
...
def render_master_table_by_periodo(df: pd.DataFrame, *, key_prefix: str = "master") -> None:
    ...
    labels = [f"{p} ({int((df[per_col].astype(str).str.strip() == p).sum())})" for p in periodos]
    tabs = st.tabs(labels)
...
def _build_export_specs(df: pd.DataFrame) -> list[dict]:
    specs: list[dict] = [{"kind": "meta", "field": field, "label": label} for field, label in _META_EXPORT]
    meta_all = _ensure_activities_meta(df)
    for formato in FORMATOS_ORDEN:
        acts = [m for m in meta_all if m["phase"] == formato]
        short = FORMATO_SHORT.get(formato, formato)
        for m in acts:
            specs.append({"kind": "act", "idx": m["idx"], "label": m["name"], "etapa": formato})
        specs.append({"kind": "pct", "field": FORMATO_PCT_COL[formato], "label": f"% {short}", "etapa": formato})
    specs.append({"kind": "gen", "field": "avance_general_vact", "label": "% Av. General Reforma"})
    return specs
...
def _write_legend_sheet(ws) -> None:
    ...
    for ri, cl in enumerate(("done", "inprog", "nostart", "devuelto", "na"), 3):
        lbl = STATUS_LABEL.get(cl, cl)
        solid = STATUS_CLR.get(cl, "9aabb5")
...
def excel_export_bytes(df: pd.DataFrame) -> bytes:
    """Genera Excel formateado con campos selector, estados y % por formato."""
    ...
    formato_blocks: list[tuple[str, list[dict]]] = []
    i = n_meta
    while i < len(specs):
        spec = specs[i]
        if spec["kind"] == "gen":
        ...
        formato = spec["etapa"]
        block: list[dict] = []
        while i < len(specs) and specs[i].get("etapa") == formato and specs[i]["kind"] != "gen":
            block.append(specs[i])
            i += 1
        formato_blocks.append((formato, block))

    gen_spec = specs[-1] if specs and specs[-1]["kind"] == "gen" else None

    for formato, block in formato_blocks:
        start_col = col
        span = len(block)
        clr = _hex6(FORMATO_CLR.get(formato, BRAND_PRIMARY))
        short = FORMATO_SHORT.get(formato, formato)
        ...
        tints = _formato_hdr_tints(FORMATO_CLR.get(formato, BRAND_PRIMARY))
