"""
Detalle de campos selector por formato en la ficha de un programa (V2).
Basado en utils/program_ficha_detalle.py, adaptado a los 11 formatos.
"""

from __future__ import annotations

import streamlit as st

from utils.charts_v2 import FORMATO_SHORT
from utils.data_loader_v2 import FORMATOS_ORDEN, FORMATO_CLR, STATUS_LABEL
from utils.poli_theme import BG_ROW, BG_ROW_ALT, BORDER_ROW, TEXT_MUTED, TEXT_PRIMARY, TEXT_SUBTLE, p_bar_html
from utils.vact_master_table import _p_esc, _status_legend_html, _vact_act_icon


def _pct_display(act: dict) -> tuple[float | None, str]:
    pct = act.get("pct")
    if isinstance(pct, (int, float)):
        return float(pct), f"{pct:.0f}%"
    return None, "—"


def _actividades_table_html(actividades: list[dict], formato_clr: str) -> str:
    if not actividades:
        return (
            f'<p style="font-size:12px;color:{TEXT_MUTED};padding:8px 0">'
            "Este formato usa un % de avance directo del Excel (sin campos selector individuales).</p>"
        )

    header = (
        f'<tr style="background:{formato_clr};color:#fff;font-size:10px;font-weight:700">'
        '<th style="padding:8px 10px;text-align:left;width:55%">Campo selector</th>'
        '<th style="padding:8px 6px;text-align:center;width:25%">Estado</th>'
        '<th style="padding:8px 8px;text-align:center;width:20%">Avance</th>'
        "</tr>"
    )
    rows = []
    for i, act in enumerate(actividades):
        bg = BG_ROW if i % 2 == 0 else BG_ROW_ALT
        cl = act.get("estado_key", "na")
        val = act.get("valor", "—")
        lbl = STATUS_LABEL.get(cl, cl)
        icon = _vact_act_icon(cl, val)
        pct_num, _ = _pct_display(act)
        avance_cell = (
            p_bar_html(pct_num) if pct_num is not None else f'<span style="color:{TEXT_MUTED}">—</span>'
        )
        rows.append(
            f'<tr style="background:{bg};border-bottom:1px solid {BORDER_ROW}">'
            f'<td style="padding:8px 10px;font-size:11px;color:{TEXT_PRIMARY};vertical-align:middle;'
            f'line-height:1.35" title="{_p_esc(act.get("nombre", ""))}">'
            f'{_p_esc(act.get("nombre", "—"))}</td>'
            f'<td style="padding:6px;text-align:center;vertical-align:middle">'
            f'<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;color:{TEXT_SUBTLE}">'
            f"{icon}<span>{_p_esc(lbl)}</span></span></td>"
            f'<td style="padding:6px 8px;text-align:center;vertical-align:middle">'
            f"{avance_cell}</td>"
            "</tr>"
        )

    return (
        f'<div style="overflow-x:auto;border:1px solid {BORDER_ROW};border-radius:8px;margin-top:4px">'
        f'<table style="width:100%;border-collapse:collapse;font-family:Segoe UI,sans-serif">'
        f"<thead>{header}</thead><tbody>{''.join(rows)}</tbody></table>"
        f"</div>"
    )


def render_program_actividades_detalle(data: dict) -> None:
    """Expanders por formato con tabla campo selector × estado × avance."""
    st.markdown(
        '<div class="f2-ficha-section">'
        '<p class="f2-ficha-section-title">Detalle de campos selector por formato</p></div>',
        unsafe_allow_html=True,
    )

    etapas_data = data.get("etapas", {})
    formato_menor = min(
        FORMATOS_ORDEN,
        key=lambda f: float(etapas_data.get(f, {}).get("pct", 0) or 0),
    )
    for formato in FORMATOS_ORDEN:
        f_data = etapas_data.get(formato, {})
        # Se conserva el orden original de las columnas del Excel.
        acts = list(f_data.get("actividades", []))
        pct_f = float(f_data.get("pct", 0) or 0)
        short = FORMATO_SHORT.get(formato, formato)
        clr = FORMATO_CLR.get(formato, "#6e7681")

        with st.expander(f"{short} — {pct_f:.0f}%", expanded=(formato == formato_menor)):
            st.markdown(_actividades_table_html(acts, clr), unsafe_allow_html=True)

    st.markdown(_status_legend_html().replace("Pulse + en cada etapa", "Referencia de estados"), unsafe_allow_html=True)
